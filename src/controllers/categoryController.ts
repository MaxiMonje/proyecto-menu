...
export const createCategory = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const files = (req.files ?? []) as Express.Multer.File[]; // ⬅️ nuevo
    const created = await categoryService.createCategoryDeep(userId, req.body, files); // ⬅️ pasa files
...
export const updateCategory = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const files = (req.files ?? []) as Express.Multer.File[]; // ⬅️ nuevo
    const updated = await categoryService.updateCategoryDeep(userId, categoryId, req.body, files); // ⬅️ pasa files
    res.json(updated);
  } catch (e) { next(e); }
};
